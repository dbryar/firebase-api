...
async function retrieve(req:express.Request,res:express.Response) {
  ...
  let message = `no data`;
  try {
    if(!formName) throw new Error(JSON.stringify({code:400,message:`Must include '/formName' in request`,trace:'PR.005'}));
    if(formName.length < 3) throw new Error(JSON.stringify({code:400,message:`Must include '/formName' in request`,trace:'PR.006'}));
    const db = admin.firestore();
    if(subId) {
